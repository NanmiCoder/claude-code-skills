...
```bash
python <srt-skill-path>/scripts/parse_srt.py "<srt_file_path>" --stats
```

**注意：** `<srt-skill-path>` 是 srt-to-structured-data skill 的安装路径，通常为 `~/.claude/plugins/srt-to-structured-data@claude-code-skills/skills/srt-to-structured-data`
...
2. 章节标题要简洁有力，便于观众快速定位
3. 最后一个章节可以是"总结"、"结语"或"彩蛋"
4. 避免章节过于密集（间隔太短会显得杂乱）
5. 时间戳取字幕段落开始时间，向下取整到秒
6. **章节总数必须 > 2 且 ≤ 10**，否则B站会拒绝
